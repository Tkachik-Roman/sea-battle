...
export default class Board {
  ...
  // Допустимость расположения корабля по текущим координатам на игровой доске
  ...
  // Отображение игрового поля: O - пустая клетка, S - корабль, X - повреждение корабля (пробоина)
  display() {
    for (let i = 0; i < this._size; i++) {
      for (let j = 0; j < this._size; j++) {
...
const shipName = 'ShipTest';
const shipLength = 3;
const shipLineUp = 0;

let ship = new Ship(shipName, shipLength, shipLineUp);
board.placeShip(ship, 0, 0);
console.log(board.boardSize + ',' + board.receiveAttack(0,1));
